...
In `section-3`, we use g++ to build the executable. For a complexe project, which use 3rd party packages and extra resources such as images, data, etc. It's just impossible to build with g++. So we need to use a build tool.
The most popular build tool 
